...
export default function NewUserManagementModel({ onClose }) {
    const {
    ...
    }, [filters, currentPage, sortConfig, searchAndFilterUsers]);
                                        ...
                                        <td className="p-5 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                {actionLoading === user.id ? (
                                                    ...
                                                    <>
                                                        <button onClick={() => setEditingUser({ ...user })} className="p-2 text-outline hover:bg-primary/10 hover:text-primary rounded-xl transition-all" title="Edit">
                                                            <Edit2 size={18} />
                                                        </button>
                                                        <button onClick={() => toggleStatus(user.id, user.active)}
                                                            className={`p-2 rounded-xl transition-all ${user.active ? 'text-outline hover:bg-amber-50 hover:text-amber-600' : 'text-green-600 hover:bg-green-50'}`}
                                                            title={user.active ? "Suspend" : "Activate"}>
                                                            {user.active ? <UserX size={18} /> : <CheckCircle2 size={18} />}
                                                        </button>
                                                        <button onClick={() => setDeleteConfirm(user.id)} className="p-2 text-outline hover:bg-red-50 hover:text-red-600 rounded-xl transition-all" title="Delete">
                                                            <Trash2 size={18} />
                                                        </button>
                                                    </>
                                                )}
